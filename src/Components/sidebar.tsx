...
import BiseccionForm from "../Forms/BiseccionForm";
...
interface SidebarProps {
  ...
  onCalcularBiseccion?: (entry: Entry) => void;
}
                ...
                <div className="px-4 pb-4 bg-gray-50 text-sm text-gray-600">
                  {selectedService === "punto-fijo" && (
                    <PuntoFijoForm
                      ...
                      canSubmit={!!entry.funcion && !!entry.x0 && !!entry.error}
                    />
                  )}
                  ...
                  {selectedService === "newton-raphson" && (
                    <NewtonRaphsonForm
                      ...
                      canSubmit={!!entry.funcion && !!entry.derivada && !!entry.x0 && !!entry.error}
                    />
                  )}
                  ...
                  {selectedService === "secante" && (
                    <SecanteForm
                      ...
                      canSubmit={!!entry.funcion && !!entry.x0 && !!entry.x1 && !!entry.error}
                    />
                  )}

                  {selectedService === "biseccion" && (
                    <BiseccionForm
                      entryId={entry.id}
                      funcion={entry.funcion || ""}
                      a={entry.a || ""}
                      ...
                      error={entry.error || ""}
                      onChange={(field, value) => onUpdateEntry(entry.id, field as keyof Entry, value)}
                      onSubmit={() => onCalcularBiseccion?.(entry)}
                      canSubmit={!!entry.funcion && !!entry.a && !!entry.b && !!entry.error}
                    />
                  )}
